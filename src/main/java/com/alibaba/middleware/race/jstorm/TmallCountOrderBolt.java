 ...
 * @date: 2016年7月2日 上午8:46:43
 */
public class TmallCountOrderBolt implements IRichBolt
{
    private static final long serialVersionUID = 7591260982890048043L;
    
    private static final Logger LOG = LoggerFactory.getLogger(TmallCountOrderBolt.class);
    
    private OutputCollector collector;
    
    private static Map<Long, Double> tmallsum = null;
    ...
    public void prepare(@SuppressWarnings("rawtypes") Map stormConf, TopologyContext context, OutputCollector collector)
    {
        ...
        tmallsum = new ConcurrentSkipListMap<Long, Double>();
            ...
            public void run()
            {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        // TODO 存储数据库
                        Iterator<?> it = tmallsum.keySet().iterator();
                        while (it.hasNext())
                        {
                            long time = (Long)it.next();
                            TairOperatorImpl.write(RaceConfig.prex_tmall + time, tmallsum.get(time));
                            // save(time, tmallsum.get(time), "tmall");
                            clearData(time, tmallsum);
                            break;
                        }
    ...
    public void execute(Tuple input)
    {
        ...
        try
        {
            ...
            if (type.equals(RaceConfig.MqTmallTradeTopic))
            {
                addPayCount(time, money, tmallsum);
    ...
    public void save(long time, double money, String str)
    {
        try
        {
            ...
            bufferWritter.write(String.valueOf(time) + "----" + money + "---" + str);
            bufferWritter.write("\n");
            bufferWritter.close();
